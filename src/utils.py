...
def read_ml(root="../..") -> list[DataFrame]:
    ...
    return [
        read_parquet(f"{root}/{ML_SILVER_PATH}/{table}.parquet")
        for table in ML_SILVER_TABLES
    ]
...
def read_bc(root="../..") -> list[DataFrame]:
    ...
    return [
        read_parquet(f"{root}/{BC_SILVER_PATH}/{table}.parquet")
        for table in BC_SILVER_TABLES
    ]


def ml_train_test_split(
    ml_ratings_df, min_user_test_samples, t_split_point=974_862_386
):
    ...
    ml_ratings_test_df = ml_ratings_test_df[
        ml_ratings_test_df["UserID"].isin(users_with_enough_ratings.index)
    ]
    ...
    ml_ratings_train_df = ml_ratings_train_df[
        ml_ratings_train_df["UserID"].isin(users_to_keep)
    ]
    ml_ratings_test_df = ml_ratings_test_df[
        ml_ratings_test_df["UserID"].isin(users_to_keep)
    ]
